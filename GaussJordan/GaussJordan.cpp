...
void GaussJordan(matriz &miMatriz){
    ...
    for(int i = 0; i < filas; i++){
        ...
        int pivoteValor = miMatriz[i][i];
        for (int j = i + 1; j < filas; j++) {
            float factor = miMatriz[j][i] / pivoteValor;
            for (int k = i; k < cols; k++) {
                miMatriz[j][k] -= factor * miMatriz[i][k];
            }
        }
        // Realizar reducción de Gauss-Jordan
        for (int j = i - 1; j >= 0; j--) {
            float factor = miMatriz[j][i] / pivoteValor;
            for (int k = i; k < cols; k++) { miMatriz[j][k] -= factor * miMatriz[i][k];
            }
        }
    }
